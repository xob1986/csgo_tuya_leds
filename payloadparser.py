    ...
    def parse_payload(self, payload, gamestate_manager):
       ...
       #gamestate_manager.gamestate.player.state.health = state_info['health'] 
        ...
        if 'player' in payload and 'state' in payload['player']:
            ...
            if 'health' in state_info and payload['player']['state']['health'] == 0 and payload['round']['phase'] == 'live':
                dead = True
                if payload['player']['state']['health'] != gamestate_manager.gamestate.player.state.health:
                    gamestate_manager.gamestate.player.state.health = state_info['health']
                    led.led_controller.Muerte()
            else:
                if payload['player']['state']['health'] != gamestate_manager.gamestate.player.state.health and payload['round']['phase'] == 'live':
                    led.led_controller.InicioPartida()
                gamestate_manager.gamestate.player.state.health = state_info['health']
            if dead == False:
                if 'flashed' in state_info and payload['player']['state']['flashed'] > 15 and gamestate_manager.gamestate.player.state.flashed == False:
                        gamestate_manager.gamestate.player.state.flashed = True
                        led.led_controller.Flash()
                else:
                    if payload['player']['state']['flashed'] <= 15 and gamestate_manager.gamestate.player.state.flashed == True:
                        gamestate_manager.gamestate.player.state.flashed = False
                        led.led_controller.InicioPartida()
                if 'burning' in state_info and payload['player']['state']['burning'] > 0 and gamestate_manager.gamestate.player.state.burning == False:
                        gamestate_manager.gamestate.player.state.burning = True
                        led.led_controller.Molo()
                else:
                    if payload['player']['state']['burning'] == 0 and gamestate_manager.gamestate.player.state.burning == True:
                        gamestate_manager.gamestate.player.state.burning = False
                        led.led_controller.InicioPartida()
                if gamestate_manager.gamestate.player.state.flashed == False and gamestate_manager.gamestate.player.state.burning == False:
                    if 'bomb' in payload['round']:
                        if payload['round']['bomb'] == 'planted':
                            led.led_controller.Planted()

            if 'round' in payload and 'phase' in payload['round'] and payload['player']['state']['health'] != 0:
                roundchanged = False
                if payload['round']['phase'] != gamestate_manager.gamestate.round_phase:
                    ...
                    if payload['round']['phase'] == 'live':
                        led.led_controller.InicioPartida()
                    if payload['round']['phase'] == 'over':
                        if payload['map']['team_ct']['consecutive_round_losses'] == 0:
                            led.led_controller.FinRondaCT()
                        else:    
                            led.led_controller.FinRondaT()
                    if payload['round']['phase'] == 'freezetime':
                        led.led_controller.Pausa()
